 ...
 */
@Controller
public class AgentAllCallsController {
    private static final Logger log = Logger.getLogger(AgentAllCallsController.class);

    @Autowired @SuppressWarnings("PackageVisibleField")
    HttpServletRequest request;

    @RequestMapping(value = "/AgentAllCallsControl", method = { RequestMethod.GET })
    public ModelAndView Get(HttpServletResponse response) throws Exception {
        ...
        ModelAndView mv = new ModelAndView("AgentAllCallsControl");
        mv.addObject("today", new SimpleDateFormat("dd/MM/yyyy").format(new Date()) );
        return mv;
    }

    @RequestMapping(value = "/AgentAllCallsControl", method = { RequestMethod.POST })
