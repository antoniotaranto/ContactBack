...
public class AgentCallController {
    private static final Logger log = Logger.getLogger(AgentCallController.class);

    @Autowired @SuppressWarnings("PackageVisibleField")
    HttpServletRequest request;

    @RequestMapping(value = "/AgentCallControl", method = { RequestMethod.GET })
    public ModelAndView Get(HttpServletResponse response) throws Exception {
        ...
        ModelAndView mv = new ModelAndView("AgentCallControl");
        mv.addObject("today", new SimpleDateFormat("dd/MM/yyyy").format(new Date()) );
        return mv;
    }

    @RequestMapping(value = "/AgentCallControl", method = { RequestMethod.POST })
