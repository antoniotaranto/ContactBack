 ...
 */
@Controller
public class DailyResumeController {
    private static final Logger log = Logger.getLogger(DailyResumeController.class);

    @Autowired @SuppressWarnings("PackageVisibleField")
    HttpServletRequest request;

    @RequestMapping(value = "/DailyResumeControl", method = { RequestMethod.GET })
    public ModelAndView Get(HttpServletResponse response) throws Exception {
        ...
        ModelAndView mv = new ModelAndView("DailyResumeControl");
        mv.addObject("today", new SimpleDateFormat("dd/MM/yyyy").format(new Date()) );
        return mv;
    }

    @RequestMapping(value = "/DailyResumeControl", method = { RequestMethod.POST })
