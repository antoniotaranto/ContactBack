 ...
 */
@Controller
public class ContactTimeResumeController {
    private static final Logger log = Logger.getLogger(ContactTimeResumeController.class);

    @Autowired @SuppressWarnings("PackageVisibleField")
    HttpServletRequest request;

    @RequestMapping(value = "/ContactTimeResumeControl", method = { RequestMethod.GET })
    public ModelAndView Get(HttpServletResponse response) throws Exception {
        ...
        ModelAndView mv = new ModelAndView("ContactTimeResumeControl");
        mv.addObject("today", new SimpleDateFormat("dd/MM/yyyy").format(new Date()) );
        return mv;
    }

    @RequestMapping(value = "/ContactTimeResumeControl", method = { RequestMethod.POST })
