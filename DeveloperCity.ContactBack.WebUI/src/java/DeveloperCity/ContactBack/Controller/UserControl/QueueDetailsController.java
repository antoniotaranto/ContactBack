...
public class QueueDetailsController {
    private static final Logger log = Logger.getLogger(QueueDetailsController.class);

    @Autowired @SuppressWarnings("PackageVisibleField")
    HttpServletRequest request;

    @RequestMapping(value = "/QueueDetailsControl", method = { RequestMethod.GET })
    public ModelAndView Get(HttpServletResponse response) throws Exception {
        ...
        ModelAndView mv = new ModelAndView("QueueDetailsControl");
        mv.addObject("today", new SimpleDateFormat("dd/MM/yyyy").format(new Date()) );
        return mv;
    }

    @RequestMapping(value = "/QueueDetailsControl", method = { RequestMethod.POST })
    public ModelAndView Post(HttpServletResponse response) throws Exception {
        ...
        Calendar calStartPlus7 = Calendar.getInstance();
        calStartPlus7.setTime(validStart);
        calStartPlus7.add(Calendar.DATE, 7);
        Calendar calEnd = Calendar.getInstance();
        calEnd.setTime(validEnd);
        if (calStartPlus7.before(calEnd)) {
            ModelAndView mv = new ModelAndView("AjaxResponse");
            mv.addObject("success", false);
            mv.addObject("errorMessage", "O intervalo máximo de dias neste relatório é de 7 dias.");
            return mv;
        }
