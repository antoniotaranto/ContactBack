 ...
 */
@Controller
public class ReturnTimeResumeController {
    private static final Logger log = Logger.getLogger(ReturnTimeResumeController.class);

    @Autowired @SuppressWarnings("PackageVisibleField")
    HttpServletRequest request;

    @RequestMapping(value = "/ReturnTimeResumeControl", method = { RequestMethod.GET })
    public ModelAndView Get(HttpServletResponse response) throws Exception {
        ...
        ModelAndView mv = new ModelAndView("ReturnTimeResumeControl");
        mv.addObject("today", new SimpleDateFormat("dd/MM/yyyy").format(new Date()) );
        return mv;
    }

    @RequestMapping(value = "/ReturnTimeResumeControl", method = { RequestMethod.POST })
